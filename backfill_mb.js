  ...
  try {
    ...
    for (let year = startYear; year <= endYear; year++) {
      for (let month = 0; month < 12; month++) {
        ...
        if (monthAppts.length > 0) { await bigquery.dataset(DATASET_ID).table('appointment_history').insert(monthAppts); stats.citas += monthAppts.length; process.stdout.write(` \x1b[32m✅ +${monthAppts.length}\x1b[0m\n`); } else { process.stdout.write(` \x1b[90m(Sin datos)\x1b[0m\n`); }
        ...
        if (monthSales.length > 0) { await bigquery.dataset(DATASET_ID).table('sales_history').insert(monthSales); stats.ventas += monthSales.length; process.stdout.write(` \x1b[32m✅ +${monthSales.length}\x1b[0m\n`); } else { process.stdout.write(` \x1b[90m(Sin datos)\x1b[0m\n`); }
